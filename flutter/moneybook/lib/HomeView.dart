...
enum _MODE{
  MODE_SHOW_MONTHLY_LIST,
  MODE_SHOW_MONTHLY_METHOD,
  MODE_SHOW_MONTHLY_USAGE,
  MODE_SHOW_SEARCH_PARAM_SET,
  MODE_SHOW_SEARCH_RESULT_LIST,
  MODE_SHOW_SEARCH_RESULT_METHOD,
  MODE_SHOW_SEARCH_RESULT_USAGE,

}
class _HomeViewState extends State<HomeView> {
  static const PAGE_DAILY = 0;
  static const PAGE_LIST = PAGE_DAILY + 1;
  ...
  Widget build(BuildContext context) {
    return  Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          BottomNavigationBarItem(
              icon: Icon(Icons.today), label: 'Daily'),
          BottomNavigationBarItem(
              icon: Icon(Icons.calendar_month), label: 'Monthly'),
        ...
        onTap: (int index) {
          setState(() {
            if (PAGE_DAILY == index) {
              _showDataMode = SHOW_CHART_DATA;
            }else if ((PAGE_LIST == _currentPage && PAGE_SEARCH == index )||
                (PAGE_DAILY==_currentPage && PAGE_LIST==index)) {
              end = begin;
              _showDataMode=SHOW_LIST_DATA;
            }
      ...
      floatingActionButton: _currentPage == PAGE_LIST ? FloatingActionButton(
  ...
  Widget? _gen_title() {
    if(PAGE_DAILY==_currentPage){
      final now=DateTime.now();
      begin=DateTime(now.year,now.month,now.day);
      end=begin.add(Duration(days: 1));
      begin=begin.subtract(Duration(days: 1));
      //print("title ${begin}");
    }else {
      if (begin == end) {
    ...
    final modeDropdown = DropdownButton<String>(
        ...
        items: dataModeCaptions.map<DropdownMenuItem<String>>(
              ...
              return DropdownMenuItem<String>(
                value: value,
                child: Icon(
                    "List".compareTo(value) == 0 ? Icons.list_alt : Icons
                        .pie_chart),
              );
            }).toList(),
        onChanged: (newValue) {
    ...
    }else if(PAGE_DAILY==_currentPage){
      return Text("Daily");
    } else {
      return Text("Config");
    }
  ...
  Widget _gen_content() {
    ...
    if (PAGE_DAILY==_currentPage || PAGE_LIST == _currentPage) {
  ...
  Widget _gen_data_view(){
    ...
    if(PAGE_DAILY==_currentPage){
      return FutureBuilder(future: trans_list, builder:
        ...
        if (snapshot.hasData) {
          ...
          daySpends[begin.day]=0;
          daySpends[begin.add(Duration(days: 1)).day]=0;
          for(var t in snapshot.data!){
            if(0>t.value.toInt()){


              if(daySpends.containsKey(t.transactionDate.day)){
                daySpends[t.transactionDate.toLocal().day]=daySpends[t.transactionDate.toLocal().day]+t.value.toInt().abs();
          ...
          return BarChart(BarChartData(
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                   ...
                   getTitlesWidget: (value,meta){
                      return Text(daySpends.keys.elementAtOrNull(value.toInt()-1).toString());
            ...
            barGroups: [
              BarChartGroupData(x: 1, barRods: [
                BarChartRodData(toY: daySpends.values.elementAtOrNull(0), width: 20),
              ]),
              BarChartGroupData(x: 2, barRods: [
                BarChartRodData(toY: daySpends.values.elementAtOrNull(1), width: 20),
              ]),
            ]
          ),
    ...
    }else if(SHOW_LIST_DATA==_showDataMode || (PAGE_SEARCH==_currentPage && SEARCH_RESULT_SHOW==_searchMode)){
